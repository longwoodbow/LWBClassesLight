...
class MakeshiftCatapultInfo : VehicleInfo
{
	Random _r(0xca7a);
	...
	void onFire(CBlob@ this, CBlob@ bullet, const u16 &in fired_charge)
	{
		...
		if (bullet !is null)
		{
			const f32 player_launch_modifier = 0.5f;// from 0.75
			const f32 other_launch_modifier = 0.7f;// from 1.1
			...
			else if(bullet.getName() == "cata_rock")//rock will be launched by normal velocity
			{
	...
	if (this.hasAttached() || this.get_bool("hadattached") || this.get_bool("facing") != this.isFacingLeft() || time_til_fire > 0)
	{
		...
		if (isClient()) //only matters visually on client
		{
			...
			if (arm !is null)
			{
				...
				const f32 armAngle = 20 + (angle / 9) + (f32(v.charge) / f32(ammo.max_charge_time)) * 15;

				Vec2f armOffset = Vec2f(-16.0f, -5.0f);
				arm.SetOffset(armOffset);

				arm.ResetTransform();
				arm.SetRelativeZ(-10.5f);
				arm.RotateBy(armAngle * (this.isFacingLeft() ? 1 : -1), Vec2f(0.5f * (this.isFacingLeft() ? -1 : 1), 16.0f));
				...
				Vec2f offset = Vec2f(2.5f, -10);
				offset.RotateBy(-armAngle, Vec2f(0.5f, 16.0f));
				offset += armOffset + Vec2f(36, 0);
				mag.offset = offset;
			}
		}
	}
...
{
	return !this.hasAttached() && this.getTeamNum() == byBlob.getTeamNum();
}